...
{
	if (pImpl->textf && pImpl->pBrushForeground_)
	{
		D2D1_POINT_2F pos = GetPosition();
		D2D1_RECT_F rect = D2D1::RectF(0,0,80,20);
		H::WriteText(target, pImpl->textf, rect, pImpl->pBrushForeground_, _T("(%.0f,%.0f)"), pos.x, pos.y);
	}
...
void F35_NS::TestGraphics::InternalInit(ID2D1RenderTarget * target)
{
	...
	pImpl->pBrushForeground_ = H::MakeSolidColorBrush(target, D2D1::ColorF(D2D1::ColorF::White, 0.3F));
}
